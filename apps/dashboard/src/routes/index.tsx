...
function App() {
  ...
  const isUsageFetching = useSpinDelay(useIsFetching({ queryKey: ['usage'] }) > 0);
  const isAlertWindowsFetching = useSpinDelay(useIsFetching({ queryKey: ['alert-windows'] }) > 0);
  const isPercentilesFetching = useSpinDelay(useIsFetching({ queryKey: ['percentiles'] }) > 0);
  const isThresholdMutating = useSpinDelay(useIsMutating({ mutationKey: ['update-alert-rule'] }) > 0, { delay: 0 });
      ...
      <Grid templateColumns={{ base: '1fr', lg: 'repeat(2, 1fr)' }} gap={6}>
        <Card.Root variant="subtle" position="relative">
          ...
          <Card.Body gap={2}>
            ...
            <UsageChart data={usage} alertWindows={alertWindows} hoveredAlertWindow={hoveredAlert} />
          </Card.Body>
        </Card.Root>
